"""Quick-start test: doc under test is ``sources/diffsynth_studio/quick_start.md``.

Run: ``python -m unittest tests.diffsynth_studio.test_quick_start_ascend -v 2>&1``

Env (injected by the engine ``quick-start-template.yml``, triggered by
``diffsynth_studio-quick-start.yml``): ``MONITORED_DOC_URL``, ``UPSTREAM_REF``,
...
class TestQuickStartAscend(MarkdownDocTestBase, unittest.TestCase):
    ...
    # Cold Hugging Face snapshot of SD 1.5 plus five inference steps.
