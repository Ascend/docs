"""Quick-start test: doc under test is ``sources/whisper_cpp/quick_start.md``.

Run: ``python -m unittest tests.whisper_cpp.test_quick_start_ascend -v 2>&1``

Env (injected by the engine ``quick-start-template.yml``, triggered by
``whisper_cpp-quick-start.yml``): ``MONITORED_DOC_URL``, ``UPSTREAM_REF``,
